...
function timeDifferenceFromNowShamsi($shamsiDate) {
    $currentGregorianDate = current_time('mysql'); 
    list($gYear, $gMonth, $gDay) = explode('-', date('Y-m-d', strtotime($currentGregorianDate)));
    list($jYearNow, $jMonthNow, $jDayNow) = gregorianToJalali($gYear, $gMonth, $gDay);
    $currentTime = date('H:i:s', strtotime($currentGregorianDate));
    $currentDateShamsi = sprintf('%04d-%02d-%02d %s', $jYearNow, $jMonthNow, $jDayNow, $currentTime);
    
    $now = new DateTime($currentDateShamsi);
    $ticketDate = new DateTime($shamsiDate);
    $interval = $now->diff($ticketDate);
